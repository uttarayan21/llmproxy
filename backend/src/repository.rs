...
impl Repository {
    ...
    pub async fn create_llm_platform(&self, user_id: i64, req: CreateLlmPlatformRequest) -> Result<LlmPlatform> {
        let result = sqlx::query(
        ...
        let platform = sqlx::query_as::<_, LlmPlatform>(
            "SELECT * FROM llm_platforms WHERE id = ?"
        )
        .bind(result.last_insert_rowid())
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn delete_llm_platform(&self, id: i64, user_id: i64) -> Result<bool> {
        let result = sqlx::query(
            "DELETE FROM llm_platforms WHERE id = ? AND user_id = ?"
        )
        .bind(id)
        .bind(user_id)
        .execute(&self.pool)
    ...
    pub async fn create_proxy_api_key(&self, user_id: i64, name: &str, key_hash: &str, key_prefix: &str) -> Result<ProxyApiKey> {
        let result = sqlx::query(
        ...
        let key = sqlx::query_as::<_, ProxyApiKey>(
            "SELECT * FROM proxy_api_keys WHERE id = ?"
        )
        .bind(result.last_insert_rowid())
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn get_proxy_api_key_by_hash(&self, key_hash: &str) -> Result<Option<ProxyApiKey>> {
        let key = sqlx::query_as::<_, ProxyApiKey>(
            "SELECT * FROM proxy_api_keys WHERE key_hash = ?"
        )
        .bind(key_hash)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn update_proxy_api_key_last_used(&self, id: i64) -> Result<()> {
        sqlx::query(
            "UPDATE proxy_api_keys SET last_used_at = datetime('now') WHERE id = ?"
        )
        .bind(id)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn delete_proxy_api_key(&self, id: i64, user_id: i64) -> Result<bool> {
        let result = sqlx::query(
            "DELETE FROM proxy_api_keys WHERE id = ? AND user_id = ?"
        )
        .bind(id)
        .bind(user_id)
        .execute(&self.pool)
